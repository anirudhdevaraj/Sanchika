...
export class CitationFormatterService {
  ...
  extractCitationData(item: Item, pageUrl: string): CitationData {
    ...
    const dateRaw =
      getFirst('dc.date.issued') ||
      getFirst('dc.date.created') ||
      getFirst('dc.date.available') ||
      getFirst('dc.date.accessioned') ||
      '';
    const year = dateRaw ? dateRaw.split('-')[0] : 'n.d.';

    const handle = (item as Item).handle;
    const suffix = handle && typeof handle === 'string' ? handle.split('/').pop() : null;
    let origin = '';
    try {
    ...
    const url = suffix && origin
      ? `${origin}/id/${suffix}`
      : (getFirst('dc.identifier.uri') || pageUrl);
  ...
  formatMLA(data: CitationData, includeDescription = false): string {
  ...
  formatChicago(data: CitationData, includeDescription = false): string {
  ...
  /**
   * MLA 9th: first author "Last, First Middle";
   * second author flips to "First Middle Last".
   * 3+ authors: first author + ", et al."
   */
  private formatAuthorsMLA(authors: string[]): string {
    if (!authors || authors.length === 0) { return ''; }
    const lastFirst = (a: string): string => {
      const parts = a.split(',').map(s => s.trim());
      return parts.length >= 2 ? `${parts[0]}, ${parts[1]}` : a;
    };
    const firstLast = (a: string): string => {
      const parts = a.split(',').map(s => s.trim());
   ...
   * 1: Last, First Middle.
   * 2-10: Last, First, First Last, and First Last.
   * 11+: Last, First, et al.
   */
  private formatAuthorsChicago(authors: string[]): string {
    ...
    if (authors.length === 1) { return lastFirst(authors[0]); }
    if (authors.length > 10) { return `${lastFirst(authors[0])}, et al`; }
    const rest = authors.slice(1, -1).map(firstLast);
    const last = firstLast(authors[authors.length - 1]);
    const head = [lastFirst(authors[0]), ...rest].join(', ');
