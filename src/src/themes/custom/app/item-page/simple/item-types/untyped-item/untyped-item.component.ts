...
export class UntypedItemComponent extends BaseComponent implements OnInit {
  ...
  get prettyIdSuffix(): string | null {
    const handle = (this.object as Item | undefined)?.handle;
  ...
  get prettyIdDisplay(): string {
    const suffix = this.prettyIdSuffix;
    if (!suffix) {
      return '';
    }
    const origin = this.doc?.location?.origin ?? '';
    const host = origin.replace(/^https?:\/\//, '');
    return host ? `${host}/id/${suffix}` : `/id/${suffix}`;
  }
