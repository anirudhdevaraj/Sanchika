...
export class SiteStatisticsPageComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Site-wide top countries & cities — served by the SSR Express proxy that
    // queries Solr statistics directly (DSpace's site-level reports don't roll
    // up child item visits).
    this.countriesReport$ = this.http.get<UsageReport>('/api/top-countries').pipe(
      catchError(() => of(undefined as unknown as UsageReport)),
      shareReplay(1),
    );

    this.citiesReport$ = this.http.get<UsageReport>('/api/top-cities').pipe(
      catchError(() => of(undefined as unknown as UsageReport)),
      shareReplay(1),
    );
