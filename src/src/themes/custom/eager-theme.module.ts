...
import { STARTS_WITH_DECORATOR_MAP } from '../../app/shared/starts-with/starts-with-decorator';
...
// Register custom theme browse-by components in the decorator map so the
// BrowseBySwitcherComponent uses our templates instead of the base ones.
BROWSE_BY_DECORATOR_MAP.get(BrowseByDataType.Metadata)?.get(DEFAULT_BROWSE_BY_CONTEXT)?.set('custom', BrowseByMetadataComponent);
BROWSE_BY_DECORATOR_MAP.get(BrowseByDataType.Title)?.get(DEFAULT_BROWSE_BY_CONTEXT)?.set('custom', BrowseByTitleComponent);
BROWSE_BY_DECORATOR_MAP.get(BrowseByDataType.Date)?.get(DEFAULT_BROWSE_BY_CONTEXT)?.set('custom', BrowseByDateComponent);

// Register custom theme starts-with components so the StartsWithLoader uses our templates.
STARTS_WITH_DECORATOR_MAP.set(StartsWithType.text, StartsWithTextComponent);
STARTS_WITH_DECORATOR_MAP.set(StartsWithType.date, StartsWithDateComponent);
